...
class Resolver:
    ...
    def connect(self):
        ...
        pref = preferences()
        RenderStudioKit.SetWorkspacePath(pref.storage_dir)
        self.export_scene()
        ...
        log("Connect to server", pref.server_url, pref.storage_url, pref.channel_id, pref.user_id, pref.storage_dir)
        info = RenderStudioKit.LiveSessionInfo(pref.server_url, pref.storage_url, pref.channel_id, pref.user_id)
    ...
    def export_scene(self):
        ...
        if not Path(self.usd_path).exists() or not self.usd_path:
            filename = bpy.path.ensure_ext(
                str(Path(f"{pref.user_id}_{Path(bpy.data.filepath).stem}")), ".usda"
            )
            self.usd_path = str(Path(pref.storage_dir) / filename)
